...
Result<AccelData> SensorsStream::accelerometer() {
   ...
   if(x.valid()) {
      auto y { mBlackboard.uint32Value(mAltTarget, mDevice, streaming_service_data_notify, 4, imu_accel_gyro_token).get() };
      auto z { mBlackboard.uint32Value(mAltTarget, mDevice, streaming_service_data_notify, 5, imu_accel_gyro_token).get() };
...
rvr::Result<ColorStream> SensorsStream::colors() {
   ...
   if(!msg.empty()) {
      ColorStream cs { static_cast<uint8_t>(msg[0]), static_cast<uint8_t>(msg[1]), static_cast<uint8_t>(msg[2]),
...
Result<GyroData> SensorsStream::gyroscope() {
   ...
   if(x.valid()) {
      ...
      auto y { mBlackboard.uint32Value(mAltTarget, mDevice, streaming_service_data_notify, 7, imu_accel_gyro_token).get() };
      auto z { mBlackboard.uint32Value(mAltTarget, mDevice, streaming_service_data_notify, 8, imu_accel_gyro_token).get() };
...
Result<ImuData> SensorsStream::imu() {
   ...
   if(x.valid()) {
      auto y { mBlackboard.uint32Value(mAltTarget, mDevice, streaming_service_data_notify, 1, imu_accel_gyro_token).get() };
      auto z { mBlackboard.uint32Value(mAltTarget, mDevice, streaming_service_data_notify, 2, imu_accel_gyro_token).get() };
...
Result<QuatData> SensorsStream::quaternion() {
   ...
   if(w.valid()) {
      ...
      res = Result<QuatData> {
         QuatData { normalize(w.get(), out_min, out_max),  //
             normalize(x.get(), out_min, out_max),         //
...
Result<LocatorData> SensorsStream::locator() {
   ...
   if(x.valid()) {
      auto y { mBlackboard.uint32Value(mAltTarget, mDevice, streaming_service_data_notify, 4, speed_velocity_locator_token).get() };
      auto [out_min, out_max] { SensorFactors[locator_token] };
      res = LocatorData { normalize(x.get(), out_min, out_max), normalize(y, out_min, out_max) };
   }
   return res;
}
//----------------------------------------------------------------------------------------------------------------------
...
Result<VelocityData> SensorsStream::velocity() {
   ...
   if(x.valid()) {
      ...
      auto y { mBlackboard.uint32Value(mAltTarget, mDevice, streaming_service_data_notify, 2, speed_velocity_locator_token).get() };

      auto [out_min, out_max] { SensorFactors[velocity_token] };
      res = VelocityData { normalize(x.get(), out_min, out_max), normalize(y, out_min, out_max) };
   }
   return res;
}
//----------------------------------------------------------------------------------------------------------------------
ResultUInt64 SensorsStream::btTime() {
   ...
   if(lower.valid()) {
      auto upper { mBlackboard.uint32Value(mAltTarget, mDevice, streaming_service_data_notify, 0, core_time_upper_token).get() };
...
ResultUInt64 SensorsStream::nordicTime() {
   ...
   if(lower.valid()) {
      auto upper { mBlackboard.uint32Value(mTarget, mDevice, streaming_service_data_notify, 0, core_time_upper_token).get() };
      res = ResultUInt64 { (static_cast<uint64_t>(upper) << 32) + lower.get() };
   }
   return res;
}
