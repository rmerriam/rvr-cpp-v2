...
namespace rvr {
    ...
    public:
        ...
        void streamEncoders(CommandResponse const want_resp = CommandResponse::resp_on_error);
        ...
        void streamVelocityLocatorSpeed(CommandResponse const want_resp = CommandResponse::resp_on_error);
    ...
    private:
        ...
        enum Cmd : uint8_t {
            ...
            get_current_detected_color_reading = 0x37, enable_color_detection = 0x38,
            //
        ...
        std::array<NormalizeFactor, 13> SensorFactors {
        //
            ...
            NormalizeFactor { 0.0, std::numeric_limits<uint32_t>::max() }, // 11- encoders
            NormalizeFactor { -1.0, 1.0, }, // ?? - quat
        };

        template <TargetPort TP, Sensor ID> void streamConfig(CommandResponse const want_resp) {
...
//----------------------------------------------------------------------------------------------------------------------
    inline void SensorsStream::streamImuAccelGyro(CommandResponse const want_resp) {
        configureStreamingBT(rvr::RvrMsg { imu_accel_gyro_token,    //
                                 0x00, imu_token, thirty_two_bit,   //
                                 0x00, accel_token, thirty_two_bit, //
                                 0x00, gyro_token, thirty_two_bit,  //
            }, want_resp);
    }
    //----------------------------------------------------------------------------------------------------------------------
    inline void SensorsStream::streamVelocityLocatorSpeed(CommandResponse const want_resp) {
        configureStreamingBT(rvr::RvrMsg { velocity_locator_speed_token, //
                                 0x00, velocity_token, thirty_two_bit,   //
                                 0x00, locator_token, thirty_two_bit,    //
            //                                 0x00, speed_token, thirty_two_bit,      //
            },
            want_resp);
    }
    //----------------------------------------------------------------------------------------------------------------------
    inline void SensorsStream::streamEncoders(CommandResponse const want_resp) {
        configureStreamingBT(rvr::RvrMsg { encoders_stream_token, //
                                 0x00, encoders_token, thirty_two_bit,   //
            }, want_resp);
    }
//----------------------------------------------------------------------------------------------------------------------
...
//======================================================================================================================
    template <typename T>
    inline float SensorsStream::normalize(T const value, float const out_min, float const out_max) {
