...
enum struct MotorIndexes : uint8_t {
    leftMotorIndex, rightMotorIndex,
};
...
enum struct BatteryVoltageReadingTypes : uint8_t {
    calibratedAndFiltered, calibratedAndUnfiltered, uncalibratedAndUnfiltered,
};
...
enum struct LEDs : uint8_t {
    rightHeadlightRed = 0, rightHeadlightGreen = 1, rightHeadlightBlue = 2, leftHeadlightRed = 3, leftHeadlightGreen = 4,
    leftHeadlightBlue = 5, leftStatusIndicationRed = 6, leftStatusIndicationGreen = 7, leftStatusIndicationBlue = 8,
    rightStatusIndicationRed = 9, rightStatusIndicationGreen = 10, rightStatusIndicationBlue = 11, batteryDoorRearRed = 12,
    batteryDoorRearGreen = 13, batteryDoorRearBlue = 14, batteryDoorFrontRed = 15, batteryDoorFrontGreen = 16,
    batteryDoorFrontBlue = 17, powerButtonFrontRed = 18, powerButtonFrontGreen = 19, powerButtonFrontBlue = 20,
    powerButtonRearRed = 21, powerButtonRearGreen = 22, powerButtonRearBlue = 23, leftBrakelightRed = 24,
    leftBrakelightGreen = 25, leftBrakelightBlue = 26, rightBrakelightRed = 27, rightBrakelightGreen = 28,
    rightBrakelightBlue = 29, undercarriageWhite = 30,
};
...
enum struct DriveFlags : uint8_t {
    none = 0, driveReverse = 1 << 0, boost = 1 << 1, fastTurn = 1 << 2, leftDirection = 1 << 3, rightDirection = 1 << 4,
    enableDrift = 1 << 5,
};

enum struct GyroMaxFlags : uint8_t {
    none = 0, maxPlusX = 1 << 0, maxMinusX = 1 << 1, maxPlusY = 1 << 2, maxMinusY = 1 << 3, maxPlusZ = 1 << 4, maxMinusZ = 1
        << 5,
};

enum struct LocatorFlags : uint8_t {
    none = 0, autoCalibrate = 1 << 0,
};
...
typedef void (*GetMotorThermalProtectionStatusCallback_t)(
    GetMotorThermalProtectionStatusReturn_t* getMotorThermalProtectionStatus);

typedef void (*MotorThermalProtectionStatusNotifyCallback_t)(
    MotorThermalProtectionStatusNotifyReturn_t* motorThermalProtectionStatusNotify);
...
typedef void (*BatteryVoltageStateChangeNotifyCallback_t)(
    BatteryVoltageStateChangeNotifyReturn_t* batteryVoltageStateChangeNotify);

typedef void (*GetBatteryVoltageInVoltsCallback_t)(GetBatteryVoltageInVoltsReturn_t* getBatteryVoltageInVolts);

typedef void (*GetBatteryVoltageStateThresholdsCallback_t)(
    GetBatteryVoltageStateThresholdsReturn_t* getBatteryVoltageStateThresholds);

typedef void (*GetCurrentSenseAmplifierCurrentCallback_t)(
    GetCurrentSenseAmplifierCurrentReturn_t* getCurrentSenseAmplifierCurrent);
