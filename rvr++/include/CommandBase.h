...
namespace rvr {
    ...
    public:
        ...
        void cmd_data(uint8_t const cmd, RvrMsg const& data, CommandResponse const want_resp = resp_on_error);
        void cmd_data_alt(uint8_t const cmd, RvrMsg const& data, CommandResponse const want_resp = resp_on_error);
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    //----------------------------------------------------------------------------------------------------------------------
    inline void CommandBase::cmd_data(uint8_t const cmd, RvrMsg const& data, CommandResponse const want_resp) {
        RvrMsg msg { buildFlags(want_resp), mTarget, mDevice, cmd, sequence() };
        msg.insert(msg.end(), data.begin(), data.end());
        mRequest.send(msg);
    }
    //----------------------------------------------------------------------------------------------------------------------
    inline void CommandBase::cmd_data_alt(uint8_t const cmd, RvrMsg const& data, CommandResponse const want_resp) {
        RvrMsg msg { buildFlags(want_resp), mAltTarget, mDevice, cmd, sequence() };
        msg.insert(msg.end(), data.begin(), data.end());
        mRequest.send(msg);
    }
